...
Agradeço ao Prof. Ricardo Duarte pela orientação

Agradeço a meu pai, Vitor César Simão, 
\end{agradecimentos}
% ---
...
\begin{resumo}
 ...
 é a reutilização destes equipamentos, conhecida em algumas fontes como Upcycling ou
 Refurbishing. Este trabalho consiste na aplicação desse conceito em dois irrigadores
 ...
 \textbf{Palavras-chaves}: Irrigação. \textit{Upcycling}. Microcontrolador.
\end{resumo}
...
\begin{resumo}[Abstract]
 \begin{otherlanguage*}{english}
   ...
   \noindent 
   \textbf{Key-words}: Irrigation. Upcycling. Microcontroller.
 \end{otherlanguage*}
\end{resumo}
...
\begin{lstlisting}[language=Kotlin, label={enviar_alarme_vm}, caption=Função de envio de alarmes no ViewModel.] fun atualizarTimers(){ viewModelScope.launch { try { val activeAlarms = allAlarms.first().filter { alarmeSalvo(it.id).not() } // Obter alarmes pendentes if (activeAlarms.isEmpty()) { _statusMessage.value = "Nenhum alarme para enviar." return@launch }
				...
				activeAlarms.forEach { alarm ->
					try {
						// Usar o novo metodo setAlarm para enviar o objeto Configuration
...
Ao salvar as alterações (código \ref{alterar_alarme_vm}), a \texttt{SegundaTela} chama a função \texttt{editarAlarme} do ViewModel. Esta função implementa uma lógica de "encontrar e substituir" no microcontrolador. Ela envia uma requisição HTTP POST para o endpoint \texttt{/editAlarm}. Esta requisição é especial, pois envia duas informações distintas: 
\begin{enumerate} 
	\item O nome original do alarme (capturado quando a tela de edição foi aberta) como um parâmetro de consulta (@Query("nomeAntigo")). \item O objeto \texttt{Configuration} completo e modificado como o corpo da requisição (@Body alarm: Configuration). 
\end{enumerate}
...
Outra forma de mensagem de bateria é quando o microcontrolador verifica que o nível desta se encontra baixo, enviando uma mensagem através do recurso CallMeBot.
